...
impl Workspace {
    ...
    /// Replace legacy Command-based shortcuts with Option-based defaults.
    ///
    /// Returns true when the shortcut was updated.
    pub fn migrate_legacy_command_shortcut(&mut self) -> bool {
        ...
        for modifier in parts {
            if modifier == "cmd" {
                // Drop Command modifier in favour of Option
                changed = true;
                continue;
            }
        ...
        if !modifiers.iter().any(|m| m == "opt") {
            modifiers.insert(0, "opt".to_string());
            changed = true;
        }

        if changed {
            modifiers.push(key);
            self.keyboard_shortcut = modifiers.join("+");
        }
    ...
    fn validate_keyboard_shortcut(shortcut: &str) -> Result<()> {
        ...
        for modifier in modifiers {
            if !["cmd", "ctrl", "opt", "shift"].contains(modifier) {
                return Err(TilleRSError::ValidationError(format!(
                    "Invalid modifier '{}' in keyboard shortcut",
                    modifier
                ))
                .into());
            }
        ...
        if key.is_empty() || (!key.chars().all(|c| c.is_alphanumeric()) && !key.starts_with('F')) {
            return Err(TilleRSError::ValidationError(format!(
                "Invalid key '{}' in keyboard shortcut",
                key
            ))
            .into());
        }
...
mod tests {
    ...
    fn test_workspace_creation() {
        ...
        assert_eq!(workspace.keyboard_shortcut, "opt+1");
    ...
    fn test_keyboard_shortcut_validation() {
        // Valid shortcuts (Option is the default, Command remains supported for compatibility)
        assert!(Workspace::validate_keyboard_shortcut("opt+1").is_ok());
        assert!(Workspace::validate_keyboard_shortcut("opt+shift+a").is_ok());
    ...
    #[test]
    fn test_migrate_legacy_command_shortcut() {
        let request = WorkspaceCreateRequest {
            name: "Legacy".to_string(),
            description: None,
            keyboard_shortcut: "cmd+shift+2".to_string(),
            tiling_pattern_id: None,
            auto_arrange: None,
        };

        let mut workspace = Workspace::new(request, Uuid::new_v4()).unwrap();
        let changed = workspace.migrate_legacy_command_shortcut();
        assert!(changed);
        assert_eq!(workspace.keyboard_shortcut, "opt+shift+2");

        // Running migration again should be a no-op
        let changed_again = workspace.migrate_legacy_command_shortcut();
        assert!(!changed_again);
        assert_eq!(workspace.keyboard_shortcut, "opt+shift+2");
    }

    #[test]
    fn test_migrate_command_shortcut_with_existing_option() {
        let request = WorkspaceCreateRequest {
            name: "Mixed".to_string(),
            description: None,
            keyboard_shortcut: "cmd+opt+3".to_string(),
            tiling_pattern_id: None,
            auto_arrange: None,
        };
        ...
        let changed = workspace.migrate_legacy_command_shortcut();
        assert!(changed);
        assert_eq!(workspace.keyboard_shortcut, "opt+3");
    }
}
